...
combined_data <- combined_data %>% 
  group_by(trial) %>% 
  mutate(iti = tap_sec - lag(tap_sec))  # Calculate ITI as the difference from the previous tap
...
```{r save datafile}
# Save the combined data
write.csv(combined_data, file = file.path(output_dir, "combined_data.csv"), row.names = FALSE)
